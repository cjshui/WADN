...
def get_logger(filename):
    ...
    log_formatter = logging.Formatter(
        fmt="%(asctime)s [%(levelname)-5.5s] %(message)s", datefmt="%Y-%b-%d %H:%M"
    )
...
def compute_weights(d, pred, batch_size):
    ...
    for i in range(num_domains):
        domain_weights_repeat = np.repeat(
            np.reshape(domain_weights[:, i], (1, num_domains)), num_tot_sample, axis=0
        )
        masks = get_mask(d)
        masks[:, i] = -masks[:, i]
...
def msda_regulizer(features_s, features_t, moment_order=4):
    ...
    for k in range(moment_order):
        ...
        for d1 in range(n_domains):
            moment_reg = moment_reg + euclidean(
                features_power_t.mean(0), features_power_s[d1].mean(0)
            )
            for d2 in range(d1 + 1, n_domains):
                moment_reg = moment_reg + euclidean(
                    features_power_s[d1].mean(0), features_power_s[d2].mean(0)
                )
